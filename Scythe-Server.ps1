...
<#
.SYNOPSIS
    Scythe V23 "Kraken Console" — Pure PowerShell HTTP Server
...
.DESCRIPTION
    ...
    Scythe-V23.ps1 scan engine in real-time via Server-Sent Events (SSE).
...
if (-not $me.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    Write-Host '[Scythe] Requesting elevation...' -ForegroundColor Cyan
...
function Protect-ReportsDirectory {
    ...
    try {
        ...
        if ($downgraded -gt 0) {
            Write-Host ("[Scythe] reports\ locked down: {0} non-admin write rule(s) downgraded to read-only." -f $downgraded) -ForegroundColor DarkGray
        }
        return ''
...
function Show-QuarantineFootprint {
    ...
    Write-Host ("[Scythe] Quarantine vault: {0} item(s), {1} MB in {2}" -f $items.Count, $mb, $vault) -ForegroundColor $c
    if ($c -eq 'Yellow') {
        Write-Host '             Review and clear it before taking this console to another client site.' -ForegroundColor Yellow
    }
}

$script:SCAN_PS   = Join-Path $ROOT 'Scythe-V23.ps1'
...
if (-not (Test-Path $script:REPORTS)) {
    try {
        ...
        Write-Host ('[Scythe] FATAL: cannot create reports folder at ' + $script:REPORTS) -ForegroundColor Red
        Write-Host ('             ' + $_.Exception.Message) -ForegroundColor Red
        Write-Host '             The drive may be read-only or ejected. Copy Scythe to a writable location.' -ForegroundColor Yellow
        exit 1
    }
}
...
if (-not $NoHardenReports) {
    ...
    if ($aclErr) {
        Write-Host ('[Scythe] WARNING: could not lock down reports\ — ' + $aclErr) -ForegroundColor Yellow
...
if ($pruned -gt 0) { Write-Host ("[Scythe] Retention: removed {0} old server log(s), keeping the newest {1} per kind." -f $pruned, $KeepLogs) -ForegroundColor DarkGray }
...
catch { Write-Host ('[Scythe] Console transcript unavailable: ' + $_.Exception.Message) -ForegroundColor Yellow }
...
function Write-ServerFault {
    ...
    if ($Quiet) { return }
    Write-Host ("[Scythe] {0}: {1}" -f $Where, $msg) -ForegroundColor DarkYellow
}
...
if (Test-Path $mitrePath) {
    try { $script:MITRE_MAP = Get-Content -LiteralPath $mitrePath -Raw -ErrorAction Stop | ConvertFrom-Json }
        ...
        $script:MITRE_MAP = $null
        Write-Host ('[Scythe] MITRE map failed to load (findings will carry no technique): ' + $_.Exception.Message) -ForegroundColor Yellow
    }
} else {
    Write-Host ("[Scythe] MITRE map not found at $mitrePath — findings will carry no technique.") -ForegroundColor Yellow
}
...
# an X-SCYTHE-Token header is honoured too, for callers that can send one.
...
function Test-RequestAuth {
    ...
    if ([string]::IsNullOrEmpty($tok)) { $tok = $Ctx.Request.Headers['X-SCYTHE-Token'] }
...
function Get-HtmlReport {
    ...
    @"
<!doctype html><html><head><meta charset="utf-8"><title>Scythe Report — $($env:COMPUTERNAME)</title>
<style>
...
<h1>◈ SCYTHE V23 — INCIDENT REPORT</h1>
...
$script:KILL_CRITICAL_NAME_RX = '(?i)^(System|smss|csrss|wininit|winlogon|services|lsass|svchost|dwm|fontdrvhost|explorer|powershell|pwsh|conhost|RuntimeBroker|MsMpEng)(\.exe)?$|(?i)(claude|scythe)'
$script:KILL_CRITICAL_DESC_RX = '(?i)(\b(System|smss|csrss|wininit|winlogon|services|lsass|svchost|dwm|fontdrvhost|explorer|powershell|pwsh|conhost|RuntimeBroker|MsMpEng)\b|claude|scythe)'
...
# This mirrors Scythe-V23.ps1's Invoke-FixMode switch; the server is admin and is
...
$KILL_CRITICAL_NAME_RX_R = '(?i)^(System|smss|csrss|wininit|winlogon|services|lsass|svchost|dwm|fontdrvhost|explorer|powershell|pwsh|conhost|RuntimeBroker|MsMpEng)(\.exe)?$|(?i)(claude|scythe)'
$KILL_CRITICAL_DESC_RX_R = '(?i)(\b(System|smss|csrss|wininit|winlogon|services|lsass|svchost|dwm|fontdrvhost|explorer|powershell|pwsh|conhost|RuntimeBroker|MsMpEng)\b|claude|scythe)'
    ...
    switch -Regex ($path) {
        ...
        '^/api/remediate$' {
            ...
            if ($recorded) {
                ...
                if ("$now" -ne "$recorded") {
                    Write-Host ("[Scythe] REFUSED remediation: $reportName changed after the scan wrote it.") -ForegroundColor Red
                ...
                Write-Host ("[Scythe] NOTE: remediating $reportName, which this launch did not produce.") -ForegroundColor Yellow
            }
        ...
        '^/api/export/html$' {
            ...
            Write-DownloadResponse $Ctx (Get-HtmlReport) 'text/html; charset=utf-8' "scythe_report_$stamp.html"
        }

        '^/api/export/csv$' {
            $stamp = [datetime]::Now.ToString('yyyyMMdd_HHmmss')
            Write-DownloadResponse $Ctx (Get-CsvReport) 'text/csv; charset=utf-8' "scythe_findings_$stamp.csv"
        }
        ...
        '^/api/ioc$' {
            ...
            if ($method -eq 'POST') {
                ...
                try {
                    ...
                    $lines.Add('# Scythe custom IOCs — generated by IOC Manager')
...
try { $Listener.Start() }
    ...
    Write-Host ('[Scythe] Listener blocked (' + $_.Exception.Message + '); adding a TEMPORARY URL ACL...') -ForegroundColor Yellow
    $acl = "http://127.0.0.1:$Port/"
    $aclOut = (& netsh http add urlacl url=$acl "user=$env:USERDOMAIN\$env:USERNAME" 2>&1) -join ' '
    if ($LASTEXITCODE -ne 0) {
        Write-Host ('[Scythe] netsh http add urlacl FAILED: ' + $aclOut) -ForegroundColor Red
    } else {
        $script:URLACL_ADDED = $acl
        Write-Host ('[Scythe] URL ACL added; it will be removed when this console exits.') -ForegroundColor DarkGray
    }
    try { $Listener.Start() }
    catch {
        Write-Host ('[Scythe] Failed to start listener after URL ACL: ' + $_.Exception.Message) -ForegroundColor Red
        if ($script:URLACL_ADDED) { & netsh http delete urlacl url=$script:URLACL_ADDED | Out-Null }
        exit 1
    }
}
catch {
    Write-Host ('[Scythe] Failed to start listener: ' + $_.Exception.Message) -ForegroundColor Red
    exit 1
}
...
Write-Host "  ║    SCYTHE V23 — KRAKEN CONSOLE               ║" -ForegroundColor Cyan
Write-Host "  ║    HTTP Server: $($Url.PadRight(34))║" -ForegroundColor Cyan
Write-Host "  ║    Press Ctrl+C to stop                          ║" -ForegroundColor Cyan
Write-Host "  ╚══════════════════════════════════════════════════╝" -ForegroundColor Cyan
Write-Host ""
Write-Host ('[Scythe] Serving GUI at ' + $Url) -ForegroundColor Green
Write-Host ('[Scythe] Scan engine: ' + $script:SCAN_PS) -ForegroundColor DarkCyan
Write-Host ''
Write-Host '  [Scythe] This console requires a launch token. Open THIS exact URL:' -ForegroundColor Yellow
...
try {
    while ($script:State.Listening) {
        try {
            ...
            if ($script:State.Listening) {
                Write-Host ('[Scythe] Listener error: ' + $_.Exception.Message) -ForegroundColor Red
            }
            break
        }
        catch {
            $errPath = if ($null -ne $ctx) { $ctx.Request.Url.AbsolutePath } else { 'unknown' }
            Write-Host ('[Scythe] Request error on ' + $errPath + ': ' + $_.Exception.Message) -ForegroundColor Yellow
    ...
    if ($script:URLACL_ADDED) {
        ...
        if ($LASTEXITCODE -eq 0) { Write-Host ('[Scythe] Temporary URL ACL removed.') -ForegroundColor DarkGray }
        else { Write-Host ("[Scythe] Could NOT remove the URL ACL — remove it by hand: netsh http delete urlacl url=$script:URLACL_ADDED  ($delOut)") -ForegroundColor Red }
    }
    Write-Host '[Scythe] Server stopped.' -ForegroundColor Cyan
    if ($script:EVENT_LOG)   { Write-Host ('[Scythe] Event log:   ' + $script:EVENT_LOG)   -ForegroundColor DarkCyan }
    if ($script:CONSOLE_LOG) { Write-Host ('[Scythe] Console log: ' + $script:CONSOLE_LOG) -ForegroundColor DarkCyan }
