...
if (-not ($global:MSP_MODE -or $global:NONINTERACTIVE)) { Start-Sleep -Milliseconds 800 }
$ransomNotePatterns = @("*readme*.txt","*DECRYPT*","*RECOVER*","*ransom*","*YOUR_FILES*","*HOW_TO_DECRYPT*","*!readme!*","*restore_files*","*HOW TO RECOVER*","*IMPORTANT*.txt","*help_decrypt*") + $RANSOM_NOTE_FILENAMES  # WS2: + 26 CISA-sourced known family note filenames
...
# ransom-note CONTENT constructs (catches notes that don't match a known filename).
...
if ($BYOVD_DRIVER_NAMES.Count -gt 0) {
    ...
    foreach ($drv in (Get-CimInstance Win32_SystemDriver -ErrorAction SilentlyContinue)) {
        ...
        if (Test-Path $path) {
            ...
            if ($sha -and $byovdHashSet.ContainsKey($sha)) { $hashNote = " [SHA256-confirmed]" }
        }
