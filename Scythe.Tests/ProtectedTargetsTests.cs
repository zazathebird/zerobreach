...
public class ProtectedTargetsTests
{
    ...
    {
        Assert.True(ProtectedTargets.Check(FixAction.DeleteFile, ScythePaths.ActionLogPath).Blocked);
        Assert.True(ProtectedTargets.Check(FixAction.Quarantine, Path.Combine(ScythePaths.VaultDir, "x.quar")).Blocked);
    }
