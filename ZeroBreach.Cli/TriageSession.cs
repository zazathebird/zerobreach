...
public sealed class TriageSession
{
    ...
    public ApprovedPlan? BuildAndConfirm(string symptomText, bool llmAssist)
    {
        ...
        {
            Console.WriteLine("no symptoms described — nothing to derive. Run a plain `zbscan --mode FULL` instead.");
            return null;
        }
    ...
    {
        var key = Environment.GetEnvironmentVariable("ZB_GEMINI_API_KEY");
        if (string.IsNullOrWhiteSpace(key))
        {
            Console.WriteLine("! --llm-assist: ZB_GEMINI_API_KEY is not set — cannot ask Gemini.");
