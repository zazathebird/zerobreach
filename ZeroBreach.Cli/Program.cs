...
static int RunScan(CliOptions options, bool allowChain = true)
{
    ...
    Say($"ZeroBreach Scan Engine v{Version} — mode {options.Mode}" +
    ...
    if (scanners.Count == 0)
    {
        Console.Error.WriteLine("error: no scanners found in ZeroBreach.Scanners — nothing to run");
        return 1;
    }
    ...
    var stem = Path.Combine(options.OutputDir, $"zbscan-{Environment.MachineName}-{startedUtc:yyyyMMdd-HHmmss}");
    if (stealth)
    {
        ...
        if (allowChain && !stealth && !cts.IsCancellationRequested)
        {
            ...
            if (Console.ReadLine()?.Trim().ToLowerInvariant() == "y")
            {
                ...
                if (followError is not null)
                {
                    ...
                    Console.Error.WriteLine($"  zbscan --profile \"{profilePath}\"");
                }
                else
                {
            ...
            {
                Console.WriteLine($"  run it later with: zbscan --profile \"{profilePath}\"");
            }
        }
    }
...
static Assembly LoadScannersAssembly()
{
    ...
    return Assembly.Load(new AssemblyName("ZeroBreach.Scanners"));
}
...
static int RunCategoriesCommand()
{
    ...
    if (scanners.Count == 0)
    {
        Console.Error.WriteLine("error: no scanners found in ZeroBreach.Scanners");
        return 1;
    }
...
static int RunReportCommand(CliOptions options)
{
    ...
    {
        Console.Error.WriteLine("usage: zbscan report show <file.json | file.json.gz>");
        return 1;
    }
    ScanReport report;
    try
...
static int RunRulesCommand(CliOptions options)
{
    ...
    {
        Console.Error.WriteLine("usage: zbscan rules lint <rules.json | iocs.txt>");
        return 1;
    }
...
static int RunVaultCommand(CliOptions options)
{
    ...
    switch (options.SubCommand)
    {
        ...
        default:
            Console.Error.WriteLine("usage: zbscan vault list | verify | restore <id> | purge <id>");
            return 1;
    }
}
...
static int RunLogCommand(CliOptions options)
{
    ...
    switch (options.SubCommand)
    {
        ...
        case "show":
            ...
            return 0;
        default:
            Console.Error.WriteLine("usage: zbscan log verify | zbscan log show");
            return 1;
    }
}
