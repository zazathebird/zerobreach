...
public sealed class CliOptions
{
    ...
    public static CliOptions Parse(string[] args, out string? error)
    {
        ...
        { error = "--symptoms/--symptoms-file/--llm-assist only apply to the triage command (zbscan triage ...)"; return o; }
    ...
    public const string Usage = """
        ZeroBreach Scan Engine — technician-run, local-only IR sweep

        usage:
          zbscan [scan] [options]        run a scan (default command)
          zbscan triage [options]        describe the machine's symptoms in plain text and
                                         let the engine derive a matching custom scan
          zbscan categories              list the detection categories/phases this build has
          zbscan report show <file>      re-render a saved report (.json or STEALTH .json.gz)
          zbscan rules lint <file>       validate a rules/IOC file without scanning
          zbscan vault list              list quarantined files
          zbscan vault verify            integrity-check vaulted files against their manifests
          zbscan vault restore <id>      restore a quarantined file to its original path
          zbscan vault purge <id>        PERMANENTLY delete a quarantined file (typed CONFIRM)
          zbscan log verify              verify the tamper-evident action log hash chain
          zbscan log show                print the action log
          zbscan help                    this text
        ...
        triage options (in addition to scan options):
          ...
          --llm-assist            ALSO ask Google Gemini to suggest categories (opt-in;
                                  needs ZB_GEMINI_API_KEY env var; only the symptom text
