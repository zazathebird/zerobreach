<#
.SYNOPSIS
    Shared assertion helpers for the ZeroBreach test suite. Dot-source, don't run.

.DESCRIPTION
    . "$PSScriptRoot/lib/ZbAssert.ps1"
    ...
    never the other way round. Read the collection with Get-ZbResults, write it to disk with
    Export-ZbResults (JSON — the hand-off tools/tests/New-TestReport.ps1 consumes), and end
    the run with Complete-ZbTestRun, which renders the summary and exits non-zero on any
    ...
    something is genuinely empty, phrase it as a boolean: Assert-ZbTrue ($x.Count -eq 0).
...
function Get-ZbCallSite {
    ...
    foreach ($fr in $frames) {
        ...
        if ([System.IO.Path]::GetFileName($sn) -ne 'ZbAssert.ps1') {
...
function Add-ZbResult {
    ...
    $rec = New-Object PSObject -Property @{
        Name     = $Name
        Section  = $script:ZbAssertSection
        Outcome  = $Outcome
        Expected = (Limit-ZbText -Value $Expected)
        Actual   = (Limit-ZbText -Value $Actual)
        File     = [string]$site.File
        Line     = [int]$site.Line
    }
    [void]$script:ZbAssertResults.Add($rec)
...
function Assert-ZbTrue {
    ...
    if ($null -eq $Condition) {
        Add-ZbResult -Name $Name -Outcome 'empty' -Expected 'True' -Actual '(null)'
        return
    }
    if ([bool]$Condition) {
        Add-ZbResult -Name $Name -Outcome 'pass' -Expected 'True' -Actual 'True'
    }
    else {
        Add-ZbResult -Name $Name -Outcome 'fail' -Expected 'True' -Actual 'False'
    }
}

function Assert-ZbFalse {
    param($Condition, [Parameter(Mandatory = $true)][string]$Name)
    if ($null -eq $Condition) {
        Add-ZbResult -Name $Name -Outcome 'empty' -Expected 'False' -Actual '(null)'
        return
    }
    if (-not [bool]$Condition) {
        Add-ZbResult -Name $Name -Outcome 'pass' -Expected 'False' -Actual 'False'
    }
    else {
        Add-ZbResult -Name $Name -Outcome 'fail' -Expected 'False' -Actual 'True'
    }
}
...
function Assert-ZbEqual {
    ...
    if ($null -eq $Actual -or (('' + $Actual).Length -eq 0)) {
        Add-ZbResult -Name $Name -Outcome 'empty' -Expected $Expected -Actual '(null or empty)'
        return
    }
    if (('' + $Expected) -eq ('' + $Actual)) {
        Add-ZbResult -Name $Name -Outcome 'pass' -Expected $Expected -Actual $Actual
    }
    else {
        Add-ZbResult -Name $Name -Outcome 'fail' -Expected $Expected -Actual $Actual
    }
}

function Assert-ZbContains {
    param($Collection, $Item, [Parameter(Mandatory = $true)][string]$Name)
    $arr = @($Collection)
    if ($null -eq $Collection -or $arr.Count -eq 0) {
        Add-ZbResult -Name $Name -Outcome 'empty' -Expected ('collection containing ''' + $Item + '''') -Actual '(null or empty collection)'
        return
    }
    if ($arr -contains $Item) {
        Add-ZbResult -Name $Name -Outcome 'pass' -Expected ('contains ''' + $Item + '''') -Actual ('contains ''' + $Item + '''')
    }
    else {
        Add-ZbResult -Name $Name -Outcome 'fail' -Expected ('contains ''' + $Item + '''') -Actual (@($arr | Select-Object -First 20) -join ', ')
    }
}

function Assert-ZbMatch {
    ...
    if ($null -eq $Text -or (('' + $Text).Length -eq 0)) {
        Add-ZbResult -Name $Name -Outcome 'empty' -Expected ('text matching ''' + $Pattern + '''') -Actual '(null or empty text)'
        return
    }
    if ($null -eq $Pattern -or (('' + $Pattern).Length -eq 0)) {
        Add-ZbResult -Name $Name -Outcome 'empty' -Expected 'a non-empty pattern' -Actual '(null or empty pattern)'
        return
    }
    if (('' + $Text) -match $Pattern) {
        Add-ZbResult -Name $Name -Outcome 'pass' -Expected ('matches ''' + $Pattern + '''') -Actual (Limit-ZbText -Value $Text)
    }
    else {
        Add-ZbResult -Name $Name -Outcome 'fail' -Expected ('matches ''' + $Pattern + '''') -Actual (Limit-ZbText -Value $Text)
    }
}

function Assert-ZbThrows {
    param([scriptblock]$Script, [string]$MessageLike, [Parameter(Mandatory = $true)][string]$Name)
    if ($null -eq $Script) {
        Add-ZbResult -Name $Name -Outcome 'empty' -Expected 'a script block that throws' -Actual '(null script block)'
        return
    }
    $threw = $false
    $msg = ''
    ...
    if (-not $threw) {
        Add-ZbResult -Name $Name -Outcome 'fail' -Expected 'a thrown error' -Actual 'no error was thrown'
        return
    }
    if (-not [string]::IsNullOrEmpty($MessageLike) -and -not ($msg -like $MessageLike)) {
        Add-ZbResult -Name $Name -Outcome 'fail' -Expected ('error message like ''' + $MessageLike + '''') -Actual $msg
        return
    }
    Add-ZbResult -Name $Name -Outcome 'pass' -Expected 'a thrown error' -Actual $msg
}

function Export-ZbResults {
