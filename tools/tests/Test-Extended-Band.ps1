...
$loaderRaw = Get-Content (Join-Path $root 'Scythe-V23.ps1') -Raw
...
$srvRaw = Get-Content (Join-Path $root 'Scythe-Server.ps1') -Raw
...
$loaderAst = [System.Management.Automation.Language.Parser]::ParseFile((Join-Path $root 'Scythe-V23.ps1'), [ref]$t, [ref]$e)
...
if ($authAst) {
    ...
    # SCYTHE_NOCACHE must produce a genuinely cache-free run.
    ...
    Assert-That 'SCYTHE_NOCACHE bypasses the memo'          $global:REAL_CALLS 3
    Assert-That 'SCYTHE_NOCACHE stores nothing'             $global:AUTHSIG_CACHE.Count 0
